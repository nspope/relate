...
# load topologies
mu = 1.29e-8
ts = tskit.load("sim_varne/1000/chr1.trees")
...
prior_mixture = prior_class.to_gamma_mixture(2, population_size=ne_est)
global_prior = tsdate.approx.average_gammas(prior.grid_data[:,0], prior.grid_data[:,1])
prior.grid_data[:] = [1, 0]

if not os.path.exists("anim"): os.mkdir("anim")
...
for i in range(1,30,1):
    ...
    nm = format(i, "03")
    ts_mix.dump(f"anim/chr1.tsdate.iter{nm}.trees")
